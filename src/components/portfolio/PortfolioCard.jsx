import { BsGithub } from "react-icons/bs";
import { FaGlobe } from "react-icons/fa";

const ProjectsCard = ({item:{title, desc}}) => {
  return (
    <div className="w-full p-4 xl:px-12 h-auto xl:py-10 rounded-lg flex flex-col outline outline-1">
        <div className="w-full h-[80%] overflow-hidden rounded-lg">
            <img
                className="w-full h-60 object-cover group-hover:scale-110 duration-300 cursor-pointer"
                // src={src}
                // alt="src"
            />
        </div>
        <div className="w-full mt-5 flex flex-col  gap-6">
            <div>
                <div className="flex items-center justify-between">
                    <h3 className="text-xl uppercase text-designColor font-normal">
                        {title}
                    </h3>
                    <div className="flex gap-2">
                        <span className="text-lg w-10 h-10 rounded-full bg-transparent inline-flex justify-center items-center text-gray-400 hover:text-designColor duration-300 cursor-pointer">
                            <FaGlobe />
                        </span>
                        <span className="text-lg w-10 h-10 rounded-full bg-transparent inline-flex justify-center items-center text-gray-400 hover:text-designColor duration-300 cursor-pointer">
                            <BsGithub />
                        </span>
                    </div>
                </div>
                <p className="text-sm tracking-wide mt-3">
                    {/* hover:text-designColor duration-300 */}
                    {desc}
                </p>
            </div>
        </div>
    </div>
  );
}

export default ProjectsCard